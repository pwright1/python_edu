...
class Bork:
    ...
    def insert_matched_keys(self, conn, toefl_record_id, siss_load_date, emplid, match_date, scc_temp_id):
        q = """
        insert into toefl_matched_keys(toefl_record_id, siss_load_date, emplid, match_date, scc_temp_id) values (?, ?, ?, ?, ?)
        on conflict do nothing
        """
        cur = conn.cursor()
        res = cur.execute(q, (toefl_record_id, siss_load_date, emplid, match_date, scc_temp_id))
        rows = cur.rowcount
        if rows != 1:
    ...
    def go(self, conn):
        ...
        with open(fname, "r", encoding="UTF-8") as file:
            ...
            while not done:
                ...
                toefl_file_id, filename, lines, toefl_record_id, fileline, ran, last, first, emplid, match_date, siss_load_date, scc_temp_id = fields
                ...
                self.insert_matched_keys(conn, db_toefl_record_id, siss_load_date, emplid, match_date, scc_temp_id)
                if line_count % 100 == 0:
                    conn.commit()
                line_count += 1
        conn.commit()
