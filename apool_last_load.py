...
class Bork:
    ...
    def insert(self, conn, tup):
        q = """
        insert into apool_last (emplid, appno, atype, last, first, middle, pref, suffix, dob, nid, a1, a2, city, state, postal, country, phone, email, term, year, sex, org) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    ...
    def go(self, conn):
        if len(sys.argv[1:]) != 1:
            print("use apool_last_load.py file.csv")
            sys.exit(-1)
        ...
        with open(fname, "r") as file:
            ...
            while not done:
                ...
                if nfields != expected_fields:
                    ...
                    sys.exit(1)
                emplid, appno, atype, last, first, middle, pref, suffix, dob, nid, a1, a2, city, state, postal, country, phone, email, term, year, sex, org = fields

                if len(nid) == 9 and nid[0:1] == "X":
                    nid = ""
                    
                tup = (emplid, appno, atype, last, first, middle, pref, suffix, dob, nid, a1, a2, city, state, postal, country, phone, email, term, year, sex, org)
