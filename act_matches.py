...
class Bork:
    ...
    def clear_query_pass(self, conn):
        q = "update actstage set query_pass = 0"
        cur = conn.cursor()
        res = cur.execute(q)
        conn.commit()
    ...
    def go(self, conn):
        if len(sys.argv[1:]) != 1:
            print("use act_matches.py 1 or both")
            sys.exit(-1)
        ...
        if which_match in ["1","both"]:
            ...
            cmd1 = f"{self.py} " + os.path.join(self.script_dir,"act_match.py 1")
            ret = os.system(cmd1)
        ...
        if which_match in ["both"]:
            ts_start = mydb_utils.get_db_ts()
            cmd2 = f"{self.py} " + os.path.join(self.script_dir, "act_match.py 2")
            ret = os.system(cmd2)
            ts_end = mydb_utils.get_db_ts()
    ...
    def load_db(self, which_match):
        ...
        cmd = f"{self.py} " + os.path.join(self.script_dir, "act_match_truncate.py")
        ret = os.system(cmd)

        if which_match in ["1","both"]:
            if os.path.exists("act_unmatched1.txt"):
                # you need to put the python exec first in line...
                cmd = f"{self.py} " + os.path.join(self.script_dir, "act_match_insert.py u1 act_unmatched1.txt")
                ret = os.system(cmd)
        
            if os.path.exists("act_ready1.txt"):
                cmd = f"{self.py} " + os.path.join(self.script_dir, "act_match_insert.py r1 act_ready1.txt")
                ret = os.system(cmd)

        if which_match in ["2","both"]:
            if os.path.exists("act_unmatched2.txt"):
                cmd = f"{self.py} " + os.path.join(self.script_dir, "act_match_insert.py u2 act_unmatched2.txt")
                ret = os.system(cmd)
        
            if os.path.exists("act_ready2.txt"):
                cmd = f"{self.py} " + os.path.join(self.script_dir, "act_match_insert.py r2 act_ready2.txt")
                ret = os.system(cmd)
    
    def query_db(self, which_match):
        cmd = f"{self.py} " + os.path.join(self.script_dir, f"act_match_results.py {which_match}")
        ret = os.system(cmd)
